...
def parking_spot_detail(request, parking_spot_id):
    ...
    time_slots = {key: time_slots[key] for key in time_slots if key not in newlist}
...
def booking_success(request, name,spots):
    ...
    bill=30*(len(spots))*100
    center=request.session.get('center')
    return render(request, 'success.html', {'name': name,'spots':newlist,'bill':bill,'center':center})
...
def handle_payment(request):
    ...
    center=request.session.get('center') 
    userInfo.slot_booked_time=request.session.get('selected_slot')
...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(request, 'home.html')  # You may want to customize this part
...
def deleteSpots(time):
    ...
    print("Key  Recieved:",  currentKey)
    finalTimes=remove_characters_less_than(time,currentKey)  
    print("FinalTime  Recieved:",  finalTimes)
    print("----------")
    return finalTimes
...
def get_key_from_time():
    ...
    # Get the current time in IST as a datetime object
    current_time = datetime.utcnow() + timedelta(hours=5, minutes=30)

    # Convert current_time to a formatted string
    current_time_str = current_time.strftime('%I:%M %p')
    print(current_time_str)

    # Convert current_time_str back to a datetime object
    current_time_dt = datetime.strptime(current_time_str, '%I:%M %p')

    # Find the key for the given local time slot
